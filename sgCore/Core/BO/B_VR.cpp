...
BOOL b_vr(short uka, short ukb, BOOL lea, BOOL leb)
{
	...
	w1 = (lpDA_POINT)npa->v;
	w2 = (lpDA_POINT)npb->v;
	vn1 = &(vera.v[uk1_2-1]);  vk1 = &(vera.v[uk1_2]);
	vn2 = &(verb.v[uk2_2-1]);  vk2 = &(verb.v[uk2_2]);
...
static BOOL  b_vr_ff(lpD_POINT * vn, lpD_POINT * vk)
{
  ...
  float gu;
	...
	object_number = 1;
  if ( !b_ort(vn,vk,&(npa->p[facea].v),&(npb->p[faceb].v)) )
      return FALSE;
	if ( !(edgea = b_vrr(npa,*vn,*vk,&vera,uk1_2,facea)) )
        return FALSE;
	...
	object_number = 2;
	if ( !b_ort(vn,vk,&(npb->p[faceb].v),&(npa->p[facea].v)) )
        return FALSE;
	if ( !(edgeb = b_vrr(npb,*vn,*vk,&verb,uk2_2,faceb)) )
        return FALSE;
  ...
  if (key == UNION) edgea = -edgea;
	if ( (gu = np_gru(npa,edgea,&npa->p[facea].v,&f)) == 0 )
        return FALSE;
	usa = (lpBO_USER)npa->user;
...
static BOOL  b_vr_ee(lpD_POINT * vn,lpD_POINT * vk)
{
  ...
  VNP           vnp;
	...
	object_number = 1;
	edgea = b_vran(npa,&vera,uk1_2);
	if ( !(edgea = b_diver(npa,&vera,uk1_2,edgea,*vn,*vk)) ) return FALSE;
	ir = abs(edgea);
	object_number = 2;
	edgeb = b_vran(npb,&verb,uk2_2);
	if ( !(edgeb = b_diver(npb,&verb,uk2_2,edgeb,*vn,*vk)) ) return FALSE; 
	jr = abs(edgeb);
