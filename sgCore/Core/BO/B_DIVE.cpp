...
short b_dive(lpNPW np, short edge, short v)
{
  ...
  lpNP_STR_LIST listd, listdd;
	...
	if (np->efc[iedge].fp < 0 || np->efc[iedge].fm < 0) {  
		...
		if ( (vv=np_new_vertex(npd,&np->v[v])) == 0)
            goto err;
		if ( (edged=np_divide_edge(npd,edged,vv)) == 0)
            goto err;
		user = (lpBO_USER)npd->user;
		user[edged].bnd = 0;
		user[edged].ind = 0;
		if ( !overwrite_np(&listd->bnp,vnp,npd,TNPW) )
            goto err;
		if ( !np_read_np_tmp(npd) ) return 0;
	}
	if (	(edged=np_divide_edge(np,edge,v)) == 0)
        return 0;
