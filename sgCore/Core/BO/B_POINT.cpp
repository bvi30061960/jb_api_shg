...
static int sort_function33( const void *a, const void *b);
static void b_get_point_on_arc(lpGEO_ARC arc, sgFloat alfa, lpD_POINT p);
static  void o_modify_limits_by_point11(lpD_POINT p, lpREGION_3D reg)
{
							 ...
							 lpLISTH	listh)
{
	...
	if ( np_gab_inter(&gabl, &gabb) ) { 
		...
		if ( answer == NP_OUT && code_operation == SUB ||
			...
			if ( !o_copy_obj(hobj_l, &hobj1,(char*)"") ) return FALSE;
			obj = (lpOBJ)hobj1;
				...
				switch (tmp.type) {
					case OCIRCLE:
						...
						for (i = 0; i <= num; i++) {
							...
							o_modify_limits_by_point11(&p, &gab);
						}
						ret = b_sub_arc(&geo_arc, &gab, lpsc);
						break;
					case OARC:
...
OSCAN_COD b_sub_line(lpGEO_LINE line, lpSCAN_CONTROL lpsc)
{
	...
	if ( !sort_vdim(&points, sort_function33) ) { rt = OSFALSE; goto end; }
	ip.p = v2;
	ip.type = B_IN_ON;        
