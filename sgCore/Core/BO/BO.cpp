...
static int sort_function55(  const void* a, const void* b);
...
//	init_listh(&line_listh);
	...
	if ((np_gab_inter(&gab_a,&gab_b)) == TRUE) {
		...
		if (code_operation == SUB) { 
			if ( !o_copy_obj(hobj_a, &hobj_c,(char*)"") ) return OSFALSE;
			obj_a = (lpOBJ)hobj_c;
...
*/
	...
	np_init(npwg);
	object_number = 1;
	if ( !np_init_list(&lista) ) { rt = OSFALSE; goto norm; }
	list_str = &lista;
	...
	if ((hindex_a = b_index_np(&lista,&gab_b)) == NULL) { rt = OSFALSE; goto norm; }
	num_index_a = lista.number_all;

	object_number = 2;
	if ( !np_init_list(&listb) ) { rt = OSFALSE; goto norm; }
	list_str = &listb;
	...
	if ((hindex_b = b_index_np(&listb,&gab_a)) == NULL) { rt = OSFALSE; goto norm; }
	num_index_b = listb.number_all;

	
	if ((rt = (OSCAN_COD)np_str_form(np_gab_inter, &lista, &listb)) != OSTRUE) goto norm;
	...
	npa = npwg;
	npwg->type = TNPW;
	if ( !(bo_new_user_data(npa)) ) { rt = OSFALSE; goto norm; }
	if ( (npb = creat_np_mem(TNPW,MAXNOV,MAXNOE,MAXNOF,MAXNOF,MAXNOE))
				== NULL ) { rt = OSFALSE; goto norm; }
	np_init(npb);
	if ( !(bo_new_user_data(npb)) ) { rt = OSFALSE; goto norm; }
...
//			object_number = 2;
			...
			if (!(read_np(&listb.bnp, str1.hnp, npa))) { rt = OSFALSE; goto norm; }
			if ( !check_point_object(&npa->v[1], hobj_a, &answer) ) { rt = OSFALSE; goto norm; }
			if ( answer == NP_IN ) goto fr; 
//			object_number = 1;
			read_elem(&lista.vdim,0, &str1);
			if (!(read_np(&lista.bnp, str1.hnp, npa))) { rt = OSFALSE; goto norm; }
			if ( !check_point_object(&npa->v[1], hobj_b, &answer) ) { rt = OSFALSE; goto norm; }
			if ( answer == NP_IN ) { 
				if ( !o_copy_obj(hobj_b, &hobj_c,(char*)"") ) { rt = OSFALSE; goto norm; }
				obj_a = (lpOBJ)hobj_c;
...
//			object_number = 1;
		...
		if ( answer == NP_IN ||
			(answer==NP_OUT && (code_operation==SUB || code_operation==INTER))) 
				goto norm; 
fr:
		if ( !o_copy_obj(hobj_a, &hobj_c,(char*)"") ) { rt = OSFALSE; goto norm; }
		obj_a = (lpOBJ)hobj_c;
	...
	if ( brept1 ) {
		object_number = 2;
		if ( !(b_list_del(&listb, hobj_b)) ) { rt = OSFALSE; goto norm; }
		if (ctrl_c_press) {											 
			put_message(CTRL_C_PRESS, NULL, 0);
			rt = OSBREAK;
			goto norm;
	...
	for (ii=0; ii<lista.number_all; ii++) {
		...
		if ((str1.ort & ST_SV) || (str1.ort & ST_MAX)) {
			if ( !(b_del_sv(&lista, &str1)) ) { rt = OSFALSE; goto norm; }
			str1.ort = 0;
			write_elem(&lista.vdim,ii,&str1);
		}
		if (ctrl_c_press) { 										
	...
	if ( brept1 ) {
		...
		for (ii=0; ii<listb.number_all; ii++) {
			...
			if ( (str2.ort & ST_SV) || (str2.ort & ST_MAX) ) {
				if ( !(b_del_sv(&listb, &str2)) ) { rt = OSFALSE; goto norm; }
				str2.ort = 0;
...
hINDEX b_index_np(lpNP_STR_LIST list, lpREGION_3D gab)
{
	...
	end_rw(&list->vdim);
	qsort((void*)index,i,sizeof(index[0]),(fptr)sort_function55);
	return hindex;
}
int sort_function55( const void* a, const void* b)
{
...
VNP b_get_hnp(hINDEX hindex, short num, short ident)
{
	...
	index = (lpINDEX)hindex;
	p = (lpINDEX)bsearch(&ident,index,num,sizeof(index[0]),(fptr)sort_function55);
	if ( !p ) return 0;
	hnp = p->hnp;
	return hnp;
}
...
//		step_grad (num_proc , ii);
		...
		if ( str.lab == 2 )  {               
			if (!(read_np(&list->bnp,str.hnp,npa)))
                goto err;
			
			if ( !(b_del(list,npa, &str)) )
                goto err;
			if (npa->nof == 0) str.lab = -1;     
			else             { str.lab =  1;       
				if ( !(overwrite_np(&list->bnp,str.hnp,npa,TNPW)) )
                    goto err;
			}
