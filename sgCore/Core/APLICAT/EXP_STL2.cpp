...
static BOOL stl_put_face22(lpNPW npw, short numf, short lv, short *v, short *vi);
static BOOL stl_post_face22(BOOL ret, lpNPW npw, short num_np, short numf);
static BOOL put_vertex_in_stl55(lpD_POINT v);
...
static BOOL stlout_tail(void)
{
	char str[] ={"endsolid Made_In_sgCore\r\n"};
...
/*	lpOBJ 	obj = static_cast<lpOBJ>(hobj);
	...
	for(int i = 0; i < 3*objTr->nTr; i += 3)
	{
		...
		if(!put_vertex_in_stl55(&tmpPnt))	return FALSE;
...
static OSCAN_COD stlout_geo_scan(hOBJ hobj, lpSCAN_CONTROL lpsc)
{
	...
	trian_put_face  = stl_put_face22;
	trian_post_face = stl_post_face22;
	if (lpsc->m) {
...
lpBUFFER_DAT bd = &stl_info->bd;
	...
	if(!story_data(bd, strlen(str1), str1)) return FALSE;
	if(!put_vertex_in_stl55(&npw->p[numf].v))
		return FALSE;
	if(!story_data(bd, strlen(str2), str2)) return FALSE;
	for(i = 0; i < 3; i++){
		if(!story_data(bd, strlen(str3), str3)) return FALSE;
		if(!put_vertex_in_stl55(&npw->v[v[i]]))
			return FALSE;
	}
	if(!story_data(bd, strlen(str4), str4)) return FALSE;
	if(!story_data(bd, strlen(str5), str5)) return FALSE;
