...
//			step_grad (num_proc , ++n);
	  if (listh->num>0) 
		  PROGRESS(100*(++n)/listh->num, "Export object");
    }
...
char *d, *c = dxfname;
	...
	while(*c){
      ...
      #if (SG_CURRENT_PLATFORM!=SG_PLATFORM_ANDROID  && SG_CURRENT_PLATFORM!=SG_PLATFORM_LINUX)
		else if(!isascii(*c)) *c = '$';            // for ANDROID
		else if(isalpha(*c)){                      // for ANDROID
			if(islower(*c)) *c = (BYTE)_toupper(*c); // for ANDROID
		}                                          // for ANDROID
		else if((!isdigit(*c)) && (!strchr("$-_", *c))) *c = '_';     // for ANDROID
        
        else if(!isascii(*c)) *c = '$';                // for ANDROID
     #else
		else if(std::isalpha(*c,loc)){                          // for ANDROID
			if(std::islower(*c,loc)) *c = (BYTE)std::toupper(*c,loc);   // for ANDROID
		}
		else if((!std::isdigit(*c,loc)) && (!strchr("$-_", *c))) *c = '_';    // for ANDROID
     #endif
		c++;
	}
}
