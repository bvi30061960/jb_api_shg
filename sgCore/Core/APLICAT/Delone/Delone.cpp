...
{
	trFloat *pointlist;                                               /* In / out */
	trFloat *pointattributelist;                                      /* In / out */
	int *pointmarkerlist;                                          /* In / out */
	int numberofpoints;                                            /* In / out */
	int numberofpointattributes;                                   /* In / out */

	int *trianglelist;                                             /* In / out */
	trFloat *triangleattributelist;                                   /* In / out */
	trFloat *trianglearealist;                                         /* In only */
	...
	trFloat *holelist;                        /* In / pointer to array copied out */
	int numberofholes;                                      /* In / copied out */

	trFloat *regionlist;                      /* In / pointer to array copied out */
	int numberofregions;                                    /* In / copied out */

	int *edgelist;                                                 /* Out only */
	int *edgemarkerlist;            /* Not used with Voronoi diagram; out only */
	trFloat *normlist;                /* Used only with Voronoi diagram; out only */
...
typedef trFloat **triangle;            /* Really:  typedef triangle *triangle   */
...
typedef trFloat **subseg;                  /* Really:  typedef subseg *subseg   */
...
struct badtriang {
  ...
  trFloat key;                             /* cos^2 of smallest (apical) angle. */
...
struct event {
  trFloat xkey, ykey;                              /* Coordinates of the event. */
...
trFloat splitter;       /* Used to split trFloat factors for exact multiplication. */
trFloat epsilon;                             /* Floating-point machine epsilon. */
trFloat resulterrbound;
trFloat ccwerrboundA, ccwerrboundB, ccwerrboundC;
trFloat iccerrboundA, iccerrboundB, iccerrboundC;
trFloat o3derrboundA, o3derrboundB, o3derrboundC;

/* Random number seed is not constant, but I've made it global anyway.       */

SG_PTR_TO_DIGIT_LONG randomseed;                     /* Current random number seed. */
...
/* Other variables. */
  ...
  trFloat xmin, xmax, ymin, ymax;                            /* x and y bounds. */
  trFloat xminextreme;      /* Nonexistent x value used as a flag in sweepline. */
...
struct behavior {
  ...
  int nobisect;
  int steiner;
  trFloat minangle, goodangle, offconstant;
  trFloat maxarea;
...
#define decode(ptr, otri)                                                     \
	(otri).orient = (int)((SG_PTR_TO_DIGIT_LONG)(ptr)& (SG_PTR_TO_DIGIT_LONG)3l);         \
  (otri).tri = (triangle *)                                                   \
  ((SG_PTR_TO_DIGIT_LONG)(ptr) ^ (SG_PTR_TO_DIGIT_LONG)(otri).orient)


#define encode(otri)                                                          \
	(triangle)((SG_PTR_TO_DIGIT_LONG)(otri).tri | (SG_PTR_TO_DIGIT_LONG)(otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
  ((SG_PTR_TO_DIGIT_LONG)(otri).tri[6] | (SG_PTR_TO_DIGIT_LONG)2l)
...
#define infected(otri)                                                        \
	(((SG_PTR_TO_DIGIT_LONG)(otri).tri[6] & (SG_PTR_TO_DIGIT_LONG)2l) != 0l)

#define elemattribute(otri, attnum)                                           \
  ((trFloat *) (otri).tri)[m->elemattribindex + (attnum)]

#define setelemattribute(otri, attnum, value)                                 \
  ((trFloat *) (otri).tri)[m->elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(otri)  ((trFloat *) (otri).tri)[m->areaboundindex]

#define setareabound(otri, value)                                             \
  ((trFloat *) (otri).tri)[m->areaboundindex] = value
...
#define sdecode(sptr, osub)                                                   \
	(osub).ssorient = (int)((SG_PTR_TO_DIGIT_LONG)(sptr)& (SG_PTR_TO_DIGIT_LONG)1l);      \
  (osub).ss = (subseg *)                                                      \
              ((SG_PTR_TO_DIGIT_LONG) (sptr) & ~ (SG_PTR_TO_DIGIT_LONG) 3l)

#define sencode(osub)                                                         \
  (subseg) ((SG_PTR_TO_DIGIT_LONG) (osub).ss | (SG_PTR_TO_DIGIT_LONG) (osub).ssorient)
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, trFloat area)
{
  trFloat dxoa, dxda, dxod;
  trFloat dyoa, dyda, dyod;
  trFloat oalen, dalen, odlen;
  trFloat maxlen;
					...
					if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
						(argv[i][j + 1] == '.')) {
							...
							while (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
								...
								workstring[k] = '\0';
								b->minangle = (trFloat) strtod(workstring, (char **) NULL);
						} else {
							b->minangle = 20.0;
						}
				}
					...
					if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
						(argv[i][j + 1] == '.')) {
							...
							while (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
								...
								workstring[k] = '\0';
								b->maxarea = (trFloat) strtod(workstring, (char **) NULL);
								if (b->maxarea <= 0.0) {
...
void poolrestart(struct memorypool *pool)
{
  ...
  alignptr = (SG_PTR_TO_DIGIT_LONG)(pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (int *)
	  (alignptr + (SG_PTR_TO_DIGIT_LONG)pool->alignbytes -
	  (alignptr % (SG_PTR_TO_DIGIT_LONG)pool->alignbytes));
              ...
              int firstitemcount, enum wordtype wtype, int alignment)
{
  ...
  wordsize = (pool->itemwordtype == POINTER) ? sizeof(int *) : sizeof(trFloat);
...
int *poolalloc(struct memorypool *pool)
{
  ...
  if (pool->deaditemstack != (int *) NULL) {
    ...
    if (pool->itemwordtype == POINTER) {
      ...
      pool->nextitem = (int *) ((trFloat *) pool->nextitem + pool->itemwords);
    }
...
void traversalinit(struct memorypool *pool)
{
  ...
  alignptr = (SG_PTR_TO_DIGIT_LONG)(pool->pathblock + 1);
...
int *traverse(struct memorypool *pool)
{
  ...
  if (pool->itemwordtype == POINTER) {
    ...
    pool->pathitem = (int *) ((trFloat *) pool->pathitem + pool->itemwords);
  }
  pool->pathitemsleft--;
  return newitem;
}
               ...
               int subsegwords)
{
  ...
  alignptr = (SG_PTR_TO_DIGIT_LONG)m->dummytribase;
  m->dummytri = (triangle *)
	  (alignptr + (SG_PTR_TO_DIGIT_LONG)m->triangles.alignbytes -
	  (alignptr % (SG_PTR_TO_DIGIT_LONG)m->triangles.alignbytes));
  ...
  if (b->usesegments) {
	...
	alignptr = (SG_PTR_TO_DIGIT_LONG)m->dummysubbase;
    m->dummysub = (subseg *)
		(alignptr + (SG_PTR_TO_DIGIT_LONG)m->subsegs.alignbytes -
		(alignptr % (SG_PTR_TO_DIGIT_LONG)m->subsegs.alignbytes));
...
void initializevertexpool(struct mesh *m, struct behavior *b)
{
 ...
 m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(trFloat) +
                        sizeof(int) - 1) /
                       sizeof(int);
  ...
  poolinit(&m->vertices, vertexsize, VERTEXPERBLOCK,
           ...
           (sizeof(trFloat) >= sizeof(triangle)) ? FLOATINGPOINT : POINTER, 0);
}
...
void initializetrisubpools(struct mesh *m, struct behavior *b)
{
 ...
 m->highorderindex = 6 + (b->usesegments * 3);
  ...
  /*   where the index is measured in trFloats.                           */
  m->elemattribindex = (trisize + sizeof(trFloat) - 1) / sizeof(trFloat);
  /* The index within each triangle at which the maximum area constraint  */
  /*   is found, where the index is measured in trFloats.  Note that if the  */
  /*   `regionattrib' flag is set, an additional attribute will be added. */
  m->areaboundindex = m->elemattribindex + m->eextras + b->regionattrib;
  /* If triangle attributes or an area bound are needed, increase the number */
  /*   of bytes occupied by a triangle.                                      */
  if (b->vararea) {
    trisize = (m->areaboundindex + 1) * sizeof(trFloat);
  } else if (m->eextras + b->regionattrib > 0) {
    trisize = m->areaboundindex * sizeof(trFloat);
  }
...
vertex getvertex(struct mesh *m, struct behavior *b, int number)
{
  ...
  alignptr = (SG_PTR_TO_DIGIT_LONG) (getblock + 1);
  foundvertex = (vertex) (alignptr + (SG_PTR_TO_DIGIT_LONG) m->vertices.alignbytes -
                          (alignptr % (SG_PTR_TO_DIGIT_LONG) m->vertices.alignbytes));
...
{
  trFloat half;
  trFloat check, lastcheck;
  int every_other;

#ifdef CPU86
  _control87(_PC_53, _MCW_PC); /* Set FPU control word for trFloat precision. */
#endif /* CPU86 */
...
int fast_expansion_sum_zeroelim(int elen, trFloat *e, int flen, trFloat *f, trFloat *h)
{
  trFloat Q;
   trFloat Qnew;
   trFloat hh;
   trFloat bvirt;
  trFloat avirt, bround, around;
...
int scale_expansion_zeroelim(int elen, trFloat *e, trFloat b, trFloat *h)
{
   trFloat Q, sum;
  trFloat hh;
   trFloat product1;
  ...
  trFloat avirt, bround, around;
   trFloat c;
   trFloat abig;
  trFloat ahi, alo, bhi, blo;
  trFloat err1, err2, err3;
...
trFloat estimate(int elen, trFloat *e)
{
  trFloat Q;
  int eindex;
...
trFloat counterclockwiseadapt(vertex pa, vertex pb, vertex pc, trFloat detsum)
{
   trFloat acx, acy, bcx, bcy;
  trFloat acxtail, acytail, bcxtail, bcytail;
   trFloat detleft, detright;
  trFloat detlefttail, detrighttail;
  trFloat det, errbound;
  trFloat B[4], C1[8], C2[12], D[16];
   trFloat B3;
  ...
  acx = (trFloat) (pa[0] - pc[0]);
  bcx = (trFloat) (pb[0] - pc[0]);
  acy = (trFloat) (pa[1] - pc[1]);
  bcy = (trFloat) (pb[1] - pc[1]);
...
trFloat counterclockwise(struct mesh *m, struct behavior *b,
                      vertex pa, vertex pb, vertex pc)
{
  trFloat detleft, detright, det;
  trFloat detsum, errbound;
...
trFloat incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, trFloat permanent)
{
   trFloat adx, bdx, cdx, ady, bdy, cdy;
  trFloat det, errbound;

   trFloat bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  trFloat bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  trFloat bc[4], ca[4], ab[4];
   trFloat bc3, ca3, ab3;
  trFloat axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  trFloat bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  trFloat cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  trFloat abdet[64];
  int ablen;
  trFloat fin1[1152], fin2[1152];
  trFloat *finnow, *finother, *finswap;
  int finlength;

  trFloat adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
   trFloat adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  trFloat adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  trFloat aa[4], bb[4], cc[4];
   trFloat aa3, bb3, cc3;
   trFloat ti1, tj1;
  trFloat ti0, tj0;
  trFloat u[4], v[4];
   trFloat u3, v3;
  trFloat temp8[8], temp16a[16], temp16b[16], temp16c[16];
  trFloat temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  trFloat axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  trFloat bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  trFloat cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  trFloat axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  trFloat axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  trFloat axtbctt[8], aytbctt[8], bxtcatt[8];
  trFloat bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  trFloat abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  trFloat abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
   trFloat abtt3, bctt3, catt3;
  trFloat negate;
  ...
  adx = (trFloat) (pa[0] - pd[0]);
  bdx = (trFloat) (pb[0] - pd[0]);
  cdx = (trFloat) (pc[0] - pd[0]);
  ady = (trFloat) (pa[1] - pd[1]);
  bdy = (trFloat) (pb[1] - pd[1]);
  cdy = (trFloat) (pc[1] - pd[1]);
...
trFloat incircle(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd)
{
  trFloat adx, bdx, cdx, ady, bdy, cdy;
  trFloat bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  trFloat alift, blift, clift;
  trFloat det;
  trFloat permanent, errbound;
...
trFloat orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                   trFloat aheight, trFloat bheight, trFloat cheight, trFloat dheight,
                   trFloat permanent)
{
   trFloat adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  trFloat det, errbound;
  ...
  trFloat bc[4], ca[4], ab[4];
   trFloat bc3, ca3, ab3;
  trFloat adet[8], bdet[8], cdet[8];
  int alen, blen, clen;
  trFloat abdet[16];
  int ablen;
  trFloat *finnow, *finother, *finswap;
  trFloat fin1[192], fin2[192];
  int finlength;

  trFloat adxtail, bdxtail, cdxtail;
  trFloat adytail, bdytail, cdytail;
  trFloat adheighttail, bdheighttail, cdheighttail;
   trFloat at_blarge, at_clarge;
   trFloat bt_clarge, bt_alarge;
   trFloat ct_alarge, ct_blarge;
  trFloat at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
  int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
   trFloat bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
   trFloat adxt_cdy1, adxt_bdy1, bdxt_ady1;
  trFloat bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
  trFloat adxt_cdy0, adxt_bdy0, bdxt_ady0;
   trFloat bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
   trFloat adyt_cdx1, adyt_bdx1, bdyt_adx1;
  trFloat bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
  trFloat adyt_cdx0, adyt_bdx0, bdyt_adx0;
  trFloat bct[8], cat[8], abt[8];
  int bctlen, catlen, abtlen;
   trFloat bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
   trFloat adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
  trFloat bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
  trFloat adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
  trFloat u[4], v[12], w[16];
   trFloat u3;
  int vlength, wlength;
  trFloat negate;
  ...
  bdy = (trFloat) (pb[1] - pd[1]);
  cdy = (trFloat) (pc[1] - pd[1]);
  adheight = (trFloat) (aheight - dheight);
  bdheight = (trFloat) (bheight - dheight);
  cdheight = (trFloat) (cheight - dheight);
...
trFloat orient3d(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd,
              trFloat aheight, trFloat bheight, trFloat cheight, trFloat dheight)
{
  trFloat adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
...
trFloat nonregular(struct mesh *m, struct behavior *b,
                vertex pa, vertex pb, vertex pc, vertex pd)
{
  if (b->weighted == 0) {
...
void findcircumcenter(struct mesh *m, struct behavior *b,
                      ...
                      vertex circumcenter, trFloat *xi, trFloat *eta, trFloat *minedge,
                      int offcenter)
{
  trFloat xdo, ydo, xao, yao;
  trFloat dodist, aodist, dadist;
  trFloat denominator;
  trFloat dx, dy, dxoff, dyoff;
...
SG_PTR_TO_DIGIT_LONG randomnation(unsigned int choices)
{
...
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
                   trFloat angle, vertex enqapex, vertex enqorg, vertex enqdest)
{
...
int splitpermitted(struct mesh *m, struct osub *testsubseg, trFloat iradius) {
  struct osub cwsubseg, ccwsubseg, cwsubseg2, ccwsubseg2;
  struct osub testsym;
  struct osub startsubseg, nowsubseg;
  vertex suborg, dest1, dest2;
  trFloat nearestpoweroffour, seglength, prevseglength, edgelength;
...
int checkseg4encroach(struct mesh *m, struct behavior *b,
                      struct osub *testsubseg, trFloat iradius)
{
  struct otri neighbortri;
  struct osub testsym;
...
void testtriangle(struct mesh *m, struct behavior *b, struct otri *testtri)
{
  ...
  vertex anglevertex;
  trFloat dxod, dyod, dxda, dyda, dxao, dyao;
  trFloat dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
  trFloat apexlen, orglen, destlen;
  trFloat angle;
  trFloat area;
  subseg sptr;                      /* Temporary variable used by tspivot(). */
...
//                 torg[0], torg[1]);
  ...
  for (i = 0; i < sampleblocks; i++) {
    alignptr = (SG_PTR_TO_DIGIT_LONG) (sampleblock + 1);
    firsttri = (triangle *) (alignptr + (SG_PTR_TO_DIGIT_LONG) m->triangles.alignbytes
                      - (alignptr % (SG_PTR_TO_DIGIT_LONG) m->triangles.alignbytes));
...
trFloat circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, trFloat ccwabc)
{
  trFloat xac, yac, xbc, ybc, xab, yab;
  trFloat aclen2, bclen2, ablen2;
...
struct splaynode *circletopinsert(struct mesh *m, struct behavior *b,
                                  ...
                                  struct otri *newkey,
                                  vertex pa, vertex pb, vertex pc, trFloat topy)
{
  trFloat ccwabc;
  trFloat xac, yac, xbc, ybc;
  trFloat aclen2, bclen2;
  trFloat searchpoint[2];
...
#ifdef REDUCED
  if (!b->quiet) {
    //  TRACE("Constructing Delaunay triangulation by divide-and-conquer method.\n");
  }
...
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
                trFloat *triangleattriblist, trFloat *trianglearealist,
...
void regionplague(struct mesh *m, struct behavior *b,
                  trFloat attribute, trFloat area)
{
  struct otri testtri;
  struct otri neighbor;
  triangle **virusloop;
...
void carveholes(struct mesh *m, struct behavior *b, trFloat *holelist, int holes,
                trFloat *regionlist, int regions)
{
  struct otri searchtri;
  struct otri triangleloop;
...
#ifdef SINGLE
  //  TRACE("Alternatively, try recompiling me with trFloat precision\n");
...
void splitencsegs(struct mesh *m, struct behavior *b, int triflaws)
{
  ...
  vertex newvertex;
  enum insertvertexresult success;
  trFloat segmentlength, nearestpoweroftwo;
  trFloat split;
  trFloat multiplier, divisor;
...
void transfernodes(struct mesh *m, struct behavior *b, trFloat *pointlist,
                   trFloat *pointattriblist, int *pointmarkerlist,
...
void writenodes(struct mesh *m, struct behavior *b, trFloat **pointlist,
                trFloat **pointattriblist, int **pointmarkerlist)
{
  trFloat *plist;
  trFloat *palist;
  ...
  if (*pointlist == (trFloat *) NULL) {
    *pointlist = (trFloat *) trimalloc(outvertices * 2 * sizeof(trFloat));
  }
  /* Allocate memory for output vertex attributes if necessary. */
  if ((m->nextras > 0) && (*pointattriblist == (trFloat *) NULL)) {
    *pointattriblist = (trFloat *) trimalloc(outvertices * m->nextras *
                                          sizeof(trFloat));
  }
...
void writeelements(struct mesh *m, struct behavior *b,
                   int **trianglelist, trFloat **triangleattriblist)
{
  ...
  if ((m->eextras > 0) && (*triangleattriblist == (trFloat *) NULL)) {
    *triangleattriblist = (trFloat *) trimalloc(m->triangles.items * m->eextras *
                                             sizeof(trFloat));
  }
  tlist = *trianglelist;
...
void writevoronoi(struct mesh *m, struct behavior *b, trFloat **vpointlist,
                  trFloat **vpointattriblist, int **vpointmarkerlist,
                  int **vedgelist, int **vedgemarkerlist, trFloat **vnormlist)
{
  trFloat *plist;
  trFloat *palist;
  int *elist;
  ...
  if (*vpointlist == (trFloat *) NULL) {
    *vpointlist = (trFloat *) trimalloc(m->triangles.items * 2 * sizeof(trFloat));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (trFloat *) NULL) {
    *vpointattriblist = (trFloat *) trimalloc(m->triangles.items * m->nextras *
                                           sizeof(trFloat));
  }
  ...
  *vedgemarkerlist = (int *) NULL;
  ...
  if (*vnormlist == (trFloat *) NULL) {
    *vnormlist = (trFloat *) trimalloc(m->edges * 2 * sizeof(trFloat));
  }
  elist = *vedgelist;
  normlist = *vnormlist;
  coordindex = 0;
...
void quality_statistics(struct mesh *m, struct behavior *b)
{
  struct otri triangleloop;
  vertex p[3];
  trFloat cossquaretable[8];
  trFloat ratiotable[16];
  trFloat dx[3], dy[3];
  trFloat edgelength[3];
  trFloat dotproduct;
  trFloat cossquare;
  trFloat triarea;
  trFloat shortest, longest;
  trFloat trilongest2;
  trFloat smallestarea, biggestarea;
  trFloat triminaltitude2;
  trFloat minaltitude;
  trFloat triaspect2;
  trFloat worstaspect;
  trFloat smallestangle, biggestangle;
  trFloat radconst, degconst;
  int angletable[18];
  int aspecttable[16];
  int aspectindex;
  int tendegree;
  ...
  degconst = 180.0 / M_PI;
  for (i = 0; i < 8; i++) {
    cossquaretable[i] = cos(radconst * (trFloat) (i + 1));
                 ...
                 struct triangulateio *out, struct triangulateio *vorout)
{
  struct mesh m;
  struct behavior b;
  trFloat *holearray;                                        /* Array of holes. */
  trFloat *regionarray;   /* Array of regional attributes and area constraints. */
  ...
  if (b.poly || b.convex) 
  {
    ...
    if (b.nopolywritten || b.noiterationnum) {
      ...
      if (b.poly) {
        ...
        out->holelist = (trFloat *) NULL;
        out->regionlist = (trFloat *) NULL;
      }
...
CTriangulator::CTriangulator(unsigned int points_count)
{
	...
	in_data->pointlist = (trFloat *)malloc(in_data->numberofpoints * 2 * sizeof(trFloat));
	memset(in_data->pointlist,0,in_data->numberofpoints * 2 * sizeof(trFloat));
	...
	in_data->pointattributelist = (trFloat *)malloc(in_data->numberofpoints *
...
CTriangulator::CTriangulator(unsigned int points_count, const sgFloat* all_points)
{
    ...
    // RA - new tr type
    {
        for (int i=0;i<in_data->numberofpoints * 2;i++)
            in_data->pointlist[i] = (trFloat)all_points[i];
    }
    
	in_data->pointmarkerlist = (int *)malloc(in_data->numberofpoints * sizeof(int));
	memset(in_data->pointmarkerlist,0,in_data->numberofpoints * sizeof(int));
	in_data->pointattributelist = (trFloat *)malloc(in_data->numberofpoints *
		in_data->numberofpointattributes * sizeof(trFloat));
	memset(in_data->pointattributelist,0,in_data->numberofpoints *
		in_data->numberofpointattributes *
		sizeof(trFloat));
}
...
bool    CTriangulator::SetPointsCount(unsigned int points_count)
{
	...
	in_data->numberofpoints = points_count;
	in_data->numberofpointattributes = 3;
	in_data->pointlist = (trFloat *)malloc(in_data->numberofpoints * 2 * sizeof(trFloat));
	memset(in_data->pointlist,0,in_data->numberofpoints * 2 * sizeof(trFloat));
	in_data->pointmarkerlist = (int *)malloc(in_data->numberofpoints * sizeof(int));
	memset(in_data->pointmarkerlist,0,in_data->numberofpoints * sizeof(int));
									...
									const int*  points_marks/*=NULL*/)
{
	...
	if (points_marks)
	{
		for (int ii = 0; ii < in_data->numberofpoints; ii++)
			in_data->pointmarkerlist[ii] = points_marks[ii];
	...
	if (points_attrs)
    {
        ...
        // RA - new tr type
        {
            for (int i=0;i<in_data->numberofpoints*in_data->numberofpointattributes;i++)
                in_data->pointattributelist[i] = (trFloat)points_attrs[i];
        }
    }
								   ...
								   const int*  segments_marks/*=NULL*/)
{
	...
	for (int ii = 0; ii < in_data->numberofsegments * 2; ii++)
		in_data->segmentlist[ii] = all_segments[ii];
	//memcpy(in_data->segmentlist,all_segments,in_data->numberofsegments*2*sizeof(int));
	if (segments_marks)
	{
		for (int ii = 0; ii < in_data->numberofsegments; ii++)
			in_data->segmentmarkerlist[ii] = segments_marks[ii];
...
bool  CTriangulator::SetHolesCount(unsigned int holes_count)
{
	...
	in_data->holelist = (trFloat*)malloc(in_data->numberofholes * 2 * sizeof(trFloat));
	memset(in_data->holelist,0,in_data->numberofholes * 2 * sizeof(trFloat));
...
bool  CTriangulator::SetAllHoles(unsigned int holes_count, const sgFloat* all_holes)
{
    ...
    // RA - new tr type
    {
        for (int i=0;i<in_data->numberofholes * 2;i++)
            in_data->holelist[i] = (trFloat)all_holes[i];
    }
...
bool   CTriangulator::StartTriangulate()
{
	...
	try
	{
		if (triangulation_temp_flag==SG_DELAUNAY_TRIANGULATION)
			triangulate((char*)"pzqP", in_data, out_data, (struct triangulateio *) NULL);//Delone
		else
		    triangulate((char*)"pzP", in_data, out_data, (struct triangulateio *) NULL);
	}
	catch (...) 
	{
		return false;
...
const trFloat* CTriangulator::GetPointsAttributes() const
{
	struct triangulateio*  out_data = reinterpret_cast<struct triangulateio*>(m_out);
