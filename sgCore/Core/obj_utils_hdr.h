...
typedef enum
{
    M_KREST, M_POINT, M_NULL, M_LIGHT, M_ROMB, M_BPLUS, M_LPLUS, M_NUM,
} MARKER_TYPE;
...
typedef enum { OSFALSE, OSTRUE, OSBREAK, OSSKIP } OSCAN_COD;
struct a{
  OBJTYPE     type;            
  LPARAM      lparam;         
  void        *data;           
...
int   np_search_edge(lpNPW npw, lpD_POINT bv, lpD_POINT ev, int ident);
...
extern char           global_temparary_directory[MAXPATH];   // for ANDROID
