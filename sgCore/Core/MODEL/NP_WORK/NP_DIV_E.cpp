...
int np_search_edge(lpNPW npw, lpD_POINT bv, lpD_POINT ev, int ident)
{
  ...
  for (edge=1 ; edge <= npw->noe ; edge++) {
    ...
    if (npw->efc[edge].ep == ident || npw->efc[edge].em == ident) {
	  ...
	  if (dpoint_eq(&npw->v[bv_npw], bv, eps_d) && dpoint_eq(&npw->v[ev_npw], ev, eps_d))
	  {
		  eps_d = was_eps_d;
		  return  edge;
	  }
      if ( dpoint_eq(&npw->v[ev_npw],bv,eps_d) && dpoint_eq(&npw->v[bv_npw],ev,eps_d) ) 
	  { 
		  eps_d = was_eps_d;
		  return -edge;
	  }
