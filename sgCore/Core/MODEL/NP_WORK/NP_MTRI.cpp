...
static  short include_vertex(lpD_POINT p, short* indv, int *num_indv, lpVDIM coor);
...
BOOL	put_tri(lpTRI_BIAND trb, lpNPTRP trp, bool intellect_adding/*=false*/)
{
	...
	if ( trb->num_indv >= (MAXINDV-3)) {
		if ( !np_mesh3v(&trb->list_str, &trb->ident, &trb->coor, c_smooth_angle, &trb->vdtri))		
			return FALSE;
		trb->num_indv = 0;
...
BOOL	end_tri_biand(lpTRI_BIAND trb, lpLISTH listho)
 {
	BOOL	cod;
	hOBJ	hobj;
	...
	PROGRESS(-1, "Building objects");

	cod = np_mesh3v(&trb->list_str, &trb->ident, &trb->coor, c_smooth_angle,
									&trb->vdtri);
	free_vdim(&trb->vdtri);
	free_vdim(&trb->coor);
	if ( trb->indv ) SGFree(trb->indv);
	...
	PROGRESS(-1, "Objects optimization");
...
static  short include_vertex(lpD_POINT p, short* indv, int *num_indv, lpVDIM coor)
{
    ...
    short  NumIndv = *num_indv;
    
	//memcpy(&indv[MAXINDV - NumIndv - 1],&indv[MAXINDV - NumIndv], sizeof(short) * (NumIndv - j + 1));	//   j...now   // RA
	
    /******* RA *********/
    for (int i=0; i<(NumIndv - j + 1); i++)
    {
        if ((MAXINDV - NumIndv + i)<MAXINDV && (MAXINDV - NumIndv - 1 + i)<MAXINDV)
        {
            indv[MAXINDV - NumIndv - 1 + i] = indv[MAXINDV - NumIndv + i];
        }
    }
    /***************/
    
    
    indv[MAXINDV - j - 1] = NumIndv;	// j-      num_indv
