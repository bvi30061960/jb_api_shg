...
BOOL np_put(lpNPW np, lpNP_STR_LIST list)
{
	...
	str.ident = np->ident;
	str.gab = np->gab;
	if ( !(vnp = write_np(&list->bnp,np,(NPTYPE)np->type)) )
        return FALSE;
	str.hnp = vnp;
	if ( !add_elem(&list->vdim,&str) )
        return FALSE;
	list->number_all++;
	return TRUE;
}
...
//			end_grad  (num_proc , i);
		...
		PROGRESS(-1, "Object's topology generation");
...
//    			step_grad (num_proc , ni++);
          ...
          write_elem(&list->vdim,istr,&str);
				...
				for (i=0 ; i < str.len ; i++) {
					if (!(i%100))
						PROGRESS(-1, "Object's boundary representation calculation");
				...
				for (i=0 ; i < str.len ; i++) {
					...
					for (ii=bg ; ii <= np1->noe ; ii++) {
						...
						for (jj=1 ; jj <= np2->noe ; jj++) {
							...
							w1 = np2->efr[jj].bv;
							w2 = np2->efr[jj].ev;
							if ( (d1 = dist_p_l(&p1, &ln, &np2->v[w1], &pw1)) > precision) 
								continue;
							if ( (d2 = dist_p_l(&p1, &ln, &np2->v[w2], &pw2)) > precision) 
								continue;
...
//	if ( list->bnp.bd.handle != -1 )	end_rw_np(&list->bnp);
	if ( list->bnp.bd.handle != 0 )	end_rw_np(&list->bnp);
