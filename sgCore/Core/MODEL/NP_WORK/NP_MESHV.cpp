...
static  /*short*/int		find_near_tr(short lv1, short lv2, lpVDIM vdtri, lpNPTRI tr);
static  /*short*/int        find_free_tr(lpVDIM vdtri, lpNPTRI tr);
...
#pragma argsused
BOOL np_mesh3v(lpNP_STR_LIST list, int *ident, lpVDIM coor, sgFloat angle,
							lpVDIM vdtri)
{
	...
	short*		inde;//nb = NULL;		//     
	short			num_act = 0;	// -
	/*short*/int			cur_tr;//nb	= 0;	//
	/*short*/int			v1,v2,v3,nof,edge1,edge2,act_edge,lv1,lv2;
	NPTRI			tr;
	D_POINT		v;
	int     	num_proc = -1;
	/*short*/int     edge;
	/*short*/int     old_next_edge;
	/*short*/int     old_v;

	if ((inde = (short*)SGMalloc(sizeof(*inde)*(MAXNOE+1))) == NULL)
        goto err;
	if ((indv = (short*)SGMalloc(sizeof(*indv)*(MAXNOV+1))) == NULL)
        goto err;
...
static  /*short*/int  find_near_tr(short lv1, short lv2, lpVDIM vdtri, lpNPTRI tr)
{
	...
	for ( i=0; i< vdtri->num_elem; i++ ,c = 0){
		...
		if ( !trtmp || (trtmp->constr&0x80) ) continue;
		for ( j=0; j<3; j++ ) {
