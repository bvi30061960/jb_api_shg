...
static int sort_function66( const short *a, const short *b);
...
#define DIST22(i) (np1->v[i].x*ka + np1->v[i].y*kb + np1->v[i].z*kc + kd)
...
//first:
	...
	do {
		fedge = np1->c[loop].fe;
		v1 = BE(fedge,np1);  edge = fedge;    f1 = DIST22(v1);
		do {
			if (fabs(f1) > eps_d) goto mt;
			edge = SL(edge,np1); v1 = BE(edge,np1); f1 = DIST22(v1);
		} while (edge != fedge);
 //   :   face1    pl
...
*/
mt:
       // static  int isHereManyTimes = 0;
       // isHereManyTimes = 0;
        fedge = edge;
		p1 = sg(f1,eps_d);
		do {
//cnt:
			v2 = EE(edge,np1);  f2 = DIST22(v2);
      p2 = sg(f2,eps_d);
			if (p1 == p2) {
				v1 = v2;    f1 = f2;    p1 = p2;
				edge = SL(edge,np1);
...
void np_sort_ver(lpD_POINT v, lpNP_VERTEX_LIST ver, short axis)
{
	np_axis = axis;
  wv = (lpDA_POINT)v;
 	qsort((void*)ver->v,ver->uk,sizeof(NP_VERTEX),(fptr)sort_function66);
}
int sort_function66( const short *a, const short *b)
{
