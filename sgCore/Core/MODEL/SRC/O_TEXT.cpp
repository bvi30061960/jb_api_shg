...
lpFONT        fontt = NULL;
lpFONT        font  = NULL;
WORD          len, rlen = 0, tablen, sdw, hran_len, i;
int           curPos = 0;
    ...
    {
        memcpy(buf, HelveticaFontData+curPos, len);
        curPos+=len;
    }
	rlen += len;
    ...
    if (name)
    {
        if(load_data(&bd, sizeof(fdata), fdata) != sizeof(fdata))
            goto err2;
    }
    else
    {
        memcpy(fdata, HelveticaFontData+curPos, sizeof(fdata));
        curPos+=sizeof(fdata);
    }
	rlen += sizeof(fdata);
...
//  tab = ((UCHAR*)fontt) + (sizeof(FONT) - 1);
    ...
    if (name)
    {
        if(load_data(&bd, tablen, tab) != tablen)
            goto err2;
    }
    else
    {
        memcpy(tab, HelveticaFontData+curPos, tablen);
        curPos+=tablen;
    }
	rlen += tablen;
...
//nb	c_out = tab;
  ...
  tab = &(font->table[0]);
    if (name)
    {
        if(load_data(&bd, hran_len, tab + sdw) != hran_len)
            goto err2;
    }
    else
    {
        memcpy(tab + sdw, HelveticaFontData+curPos, hran_len);
        curPos+=hran_len;
    }
	if(fdata[0] == 0){ //  0-     
...
//met_inline:
	...
	for(i = FTARROW; i < FTNUMTYPES; i++){
		...
		if(!len) continue;
		if(ft_bd->idlist[i - FTARROW]) SGFree(ft_bd->idlist[i - FTARROW]);
		ft_bd->idlist[i - FTARROW] = (IDENT_V*)SGMalloc(sizeof(IDENT_V)*(len + 1));
...
IDENT_V    id;
	...
	for(i = FTARROW; i < FTNUMTYPES; i++){
		...
		else     ft_bd->idlist[i - FTARROW] = (IDENT_V*)SGMalloc(sizeof(IDENT_V)*(len + 1));
		idlist = ft_bd->idlist[i - FTARROW];
		if(!idlist) continue;
		id = ft_bd->ft_listh[i].head;
		j = 0;
