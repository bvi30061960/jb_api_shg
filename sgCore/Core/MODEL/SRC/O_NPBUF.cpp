...
*/
	{
	   ...
	   GetTempFileName(Buffer, "mrph", 0, np_buf->name);*/
		   
        
#if (SG_CURRENT_PLATFORM==SG_PLATFORM_IOS)
        strcpy(np_buf->name, tempnam("/usr/temp","mrph"));   // for ANDROID
#else
		#if (SG_CURRENT_PLATFORM==SG_PLATFORM_WINDOWS)
		    char Buffer[MAX_PATH]; 	// address of buffer for temp. path
		    GetTempPathA(MAX_PATH, Buffer );
		    GetTempFileNameA(Buffer, "mrph", 0, np_buf->name);
        #else
            strcpy(np_buf->name, tempnam(global_temparary_directory,"mrph"));
        #endif
#endif
...
VNP write_np(lpNP_BUF np_buf,lpNPW npw, NPTYPE type)
{
	...
	if ( !seek_buf(&np_buf->bd,-1) )
        return FALSE;
	np_loc.offset = get_buf_offset(&np_buf->bd);
	np_loc.size   = get_size_np(npw, type);
	if ( !write_np_buf(np_buf, npw, type) )
        return 0;
	if ( !add_elem(&np_buf->dim,&np_loc) )
        return 0;
/*
