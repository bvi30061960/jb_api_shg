...
//  
	if ( !load_blocks_v2(bd,(char*)"Load (1/2)")) goto err;
...
static BOOL load_vd_brep(lpBUFFER_DAT bd)
{
	...
	for (i=0; i<numbrep; i++) {
		...
		if ( lnp.kind != COMMON ) {
            ...
            sgFloat* tmpData = (sgFloat*)malloc(csg->size);
            memcpy(tmpData, csg->data, csg->size);
			lnp.num_np = create_primitive_np((BREPKIND)lnp.kind,&lnp.listh, /*(sgFloat *) csg->data*/ tmpData);    // RA
            free(tmpData);
			if ( !lnp.num_np ) goto err;
			if ( !add_elem(&vd_brep,&lnp) ) goto err;
			continue;
		}
		if (global_loadedFileHeader.major_ver == KERNEL_VERSION_MAJOR)
		{
		...
		{
			short tmpShrt = 0;
			if (load_data(bd, sizeof(tmpShrt), &tmpShrt) != sizeof(tmpShrt))
				goto err;
			lnp.num_np = tmpShrt;
		}
		init_vld(&vld);
...
short         errcode;
	...
	while(TRUE){
		...
		//   
		for(i = 0; i < rec.num; i++){
			if((lpid = (IDENT_V*)get_elem(vd_ident, irec[i].attr - 1)) == NULL) attr_exit();
			irec[i].attr = *lpid;
			if(irec[i].item){
				if((lpid = (IDENT_V*)get_elem(vd_ident, irec[i].item - 1)) == NULL) attr_exit();
				irec[i].item = *lpid;
			}
		}
...
BOOL load_np(lpBUFFER_DAT bd)
{
	...
	{
		short tmpShrt = 0;
		if (load_data(bd, sizeof(tmpShrt), &tmpShrt) != sizeof(tmpShrt)) return FALSE;
		n->ident = tmpShrt;
	}
