...
static  OSCAN_COD  (**apr_typeg3)(lpOBJ obj,lpSCAN_CONTROL lpsc);
...
static BOOL apr_pipe(lpMESHDD mdd, hOBJ hobj, lpFORM form, lpMATR matr){
	...
	apr_typeg3 = (OSCAN_COD (**)(lpOBJ obj,lpSCAN_CONTROL lpsc))GetMethodArray(OMT_PIPE_R);

	apr_typeg3[OLINE]  = part_line;		// OLINE
	apr_typeg3[OCIRCLE]= part_circle;	// OCIRCLE
	apr_typeg3[OARC]   = part_arc;		// OARC
	apr_typeg3[OSPLINE]= part_spline;	// OSPLINE
...
static  OSCAN_COD apr_geo_scan(hOBJ hobj,lpSCAN_CONTROL lpsc){
	...
	obj = (lpOBJ)hobj;
	cod = apr_typeg3[lpsc->type](obj,lpsc);
