...
BOOL   ret=FALSE;
short	 i;
sgFloat *N_i = NULL, *N_i1 = NULL, *N_j = NULL;

    N_i=(sgFloat*)SGMalloc(srf_dat->P.n*sizeof(sgFloat));
	if(N_i==NULL) return FALSE;

    N_i1=(sgFloat*)SGMalloc((srf_dat->P.n-1)*sizeof(sgFloat));
	if(N_i1==NULL) goto err;

    N_j=(sgFloat*)SGMalloc(srf_dat->P.m*sizeof(sgFloat));
	if(N_j==NULL) goto err;
