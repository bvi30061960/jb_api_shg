...
#if (SG_CURRENT_PLATFORM!=SG_PLATFORM_ANDROID &&  SG_CURRENT_PLATFORM!=SG_PLATFORM_LINUX)
	if(isascii(c))if(isupper(c))      // for ANDROID
		return (BYTE)(_tolower(c));     // for ANDROID
#else
    if(true)  if(std::isupper(c,loc))        // for ANDROID
        return (BYTE)(std::tolower(c,loc));     // for ANDROID
#endif
	return c;
}
...
char* d;
  if(c && (d = strchr(rus_dn, c)) != NULL)
    ...
    if(isascii(c))if(islower(c))     // for ANDROID
       return (BYTE)(_toupper(c));   // for ANDROID
#else
   
    if(true)if(std::islower(c,loc))     // for ANDROID
        return (BYTE)(std::toupper(c,loc));   // for ANDROID
#endif
...
#if (SG_CURRENT_PLATFORM!=SG_PLATFORM_ANDROID &&  SG_CURRENT_PLATFORM!=SG_PLATFORM_LINUX)
	if(isascii(c))if(isupper(c))        return TRUE;   // for ANDROID
#else
    if(true)   if(std::isupper(c,loc))        return TRUE;   // for ANDROID
#endif
	return FALSE;
}
...
#if (SG_CURRENT_PLATFORM!=SG_PLATFORM_ANDROID &&  SG_CURRENT_PLATFORM!=SG_PLATFORM_LINUX)
	if(isascii(c))if(islower(c))        return TRUE;    // for ANDROID
#else
    if(true)  if(std::islower(c,loc))        return TRUE;    // for ANDROID
#endif
  return FALSE;
}
...
{
#if (SG_CURRENT_PLATFORM!=SG_PLATFORM_ANDROID &&  SG_CURRENT_PLATFORM!=SG_PLATFORM_LINUX)
  if(isalpha(c)) return TRUE;   // for ANDROID
#else
    if(std::isalpha(c,loc)) return TRUE;   // for ANDROID
#endif
...
{
#if (SG_CURRENT_PLATFORM!=SG_PLATFORM_ANDROID &&  SG_CURRENT_PLATFORM!=SG_PLATFORM_LINUX)
  if(isdigit(c)) return TRUE; //  for ANDROID
#else
    if(std::isdigit(c,loc)) return TRUE; //  for ANDROID
#endif
...
{
  return z_isalpha_ext(c, (char*)ID_ALFA_EXT);
}

BOOL z_isalnum_id(char c)

{
  return z_isalnum_ext(c, (char*)ID_ALFA_EXT);
}
