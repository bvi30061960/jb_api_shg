...
static  VADR get_hpage(lpVDIM dim, int num_page);
...
BOOL add_elem(lpVDIM dim,void * buf)
{
  ...
  VADR  hpage;
    ...
    num = (int)(dim->num_elem %  dim->num_elem_page);
	if ( !num )
		if ( !add_dim_page(dim) )
            return FALSE;
	num_page = (dim->num_elem / dim->num_elem_page);
...
once:
	num = (dim->num_elem %  dim->num_elem_page);
	num_page = (dim->num_elem / dim->num_elem_page);
...
BOOL write_elem(lpVDIM dim,int num, void * data)
{
	...
	num_page = (num / dim->num_elem_page);
	num     %= dim->num_elem_page;
	hpage = get_hpage(dim,num_page);
...
void * get_next_elem(lpVDIM dim)
{
  ...
  if ( dim->cur_elem >= dim->num_elem )  goto err;
	if ( !(dim->cur_elem % dim->num_elem_page) || !dim->elem) {
		num_page = (dim->cur_elem / dim->num_elem_page);
		num      = (dim->cur_elem % dim->num_elem_page);
...
static  BOOL add_dim_page(lpVDIM dim)
{
	...
	if ( !dim->hpage_a )  {  //   
		if ( (dim->hpage_a = alloc_page()) == NULL )
            goto err;
		dim->num_add_pages = 1;
	}
	num_page -= NUM_VDIM_PAGE;
	/*if ( num_page >= (int)SIZE_VPAGE/sizeof(VADR) ) {
		u_handler_err(UE_OVERFLOW_VDIM);
		return FALSE;
	}*/
	if (num_page >= (int)((SIZE_VPAGE * dim->num_add_pages) / sizeof(VADR))) {
		p = (VADR*)SGRealloc(dim->hpage_a, SIZE_VPAGE * (dim->num_add_pages + 1));
		if (!p) {
			u_handler_err(UE_OVERFLOW_VDIM);
			goto err;	// 09.03.2007 Øóáèí Â.Ï.
		}
		dim->hpage_a = p;
		dim->num_add_pages++;
	}
...
{
	int   i, num_page, num_page_all;
	VADR * pa;

	if ( number >= dim->num_elem )  return;
	if (number < 0)  number = 0;
	num_page = (number / dim->num_elem_page);
	...
	if (num_page <= NUM_VDIM_PAGE) 
	{
			...
			if (dim->hpage_a) 
			{
				num_page = (dim->num_elem / dim->num_elem_page /*+ 1*/);
				num_page -= NUM_VDIM_PAGE;
				pa = (VADR *)dim->hpage_a;
	...
	{
		num_page_all = dim->num_elem / dim->num_elem_page;
		if (dim->num_elem % dim->num_elem_page)   num_page_all++;
		num_page -= NUM_VDIM_PAGE;
		num_page_all -= NUM_VDIM_PAGE;
		pa = (VADR *)dim->hpage_a;
		for (i = num_page; i < num_page_all; i++) {
			if (!pa[i]) continue;
			SGFree(pa[i]);
			pa[i] = NULL;
		}
...
//																				const void * elem2))
{
	int   i,j,num_page, tmp, num_page1;
    short num_elem_last;
	char	* p;
	SORT_ELEM* fe;
	VDIM	vd_new;
	...
	num_page = (vdim->num_elem / vdim->num_elem_page);
