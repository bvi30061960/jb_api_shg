...
#define EQ(a, b)           (fabs((a) - (b)) <= SG_EPSILON)
...
{
  sg_vertex          vertex;       /* Piggy-backed contour vertex data  */
  sg_vertex          bot;          /* Edge lower (x, y) coordinate      */
  sg_vertex          top;          /* Edge upper (x, y) coordinate      */
...
typedef struct it_shape             /* Intersection table                */
{
  ...
  sg_vertex          point;        /* Point of intersection             */
...
static int count_optimal_vertices(sg_vertex_list c)
{
  int result= 0, i;
...
static edge_node *build_lmt(lmt_node **lmt, sb_tree **sbtree,
                            int *sbt_entries, sg_polygon *p, int type,
                            sg_op op)
{
      ...
      for (min= 0; min < num_vertices; min++)
      {
        ...
        if (FWD_MIN(edge_table, min, num_vertices))
        {
          ...
          for (i= 0; i < num_edges; i++)
          {
            ...
            e[i].next_bound= NULL;
            e[i].bside[CLIP]= (op == SG_DIFF) ? RIGHT : LEFT;
            e[i].bside[SUBJ]= LEFT;
          }
...
static bbox *create_contour_bboxes(sg_polygon *p)
{
  bbox *box;
  int   c, v;
...
static void minimax_test(sg_polygon *subj, sg_polygon *clip, sg_op op)
{
  bbox *s_bbox, *c_bbox;
...
void sg_free_polygon(sg_polygon *p)
{
  int c;
...
void sg_read_polygon(FILE *fp, int read_hole_flags, sg_polygon *p)
{
  int c, v;
  ...
  MALLOC(p->contour, p->num_contours
         * sizeof(sg_vertex_list), "contour creation", sg_vertex_list);
  for (c= 0; c < p->num_contours; c++)
  {
    ...
    MALLOC(p->contour[c].vertex, p->contour[c].num_vertices
           * sizeof(sg_vertex), "vertex creation", sg_vertex);
    for (v= 0; v < p->contour[c].num_vertices; v++)
...
void sg_write_polygon(FILE *fp, int write_hole_flags, sg_polygon *p)
{
  int c, v;
...
void sg_add_contour(sg_polygon *p, sg_vertex_list *new_contour, int hole)
{
  int             *extended_hole, c, v;
  sg_vertex_list *extended_contour;
  ...
  MALLOC(extended_contour, (p->num_contours + 1)
         * sizeof(sg_vertex_list), "contour addition", sg_vertex_list);
  ...
  MALLOC(extended_contour[c].vertex, new_contour->num_vertices
         * sizeof(sg_vertex), "contour addition", sg_vertex);
...
void sg_polygon_clip(sg_op op, sg_polygon *subj, sg_polygon *clip,
                      sg_polygon *result)
{
  sb_tree       *sbtree= NULL;
  ...
  long double        *sbt= NULL, xb, px, yb, yt, dy, ix, iy;

  /* Test for trivial NULL result cases */
  if (((subj->num_contours == 0) && (clip->num_contours == 0))
   || ((subj->num_contours == 0) && ((op == SG_INT) || (op == SG_DIFF)))
   || ((clip->num_contours == 0) &&  (op == SG_INT)))
  {
  ...
  if (subj == result)
    sg_free_polygon(subj);
  if (clip == result)
    sg_free_polygon(clip);
    ...
    for (poly= out_poly; poly; poly= npoly)
    {
      ...
      if (poly->active)
      {
        ...
        MALLOC(result->contour[c].vertex,
          result->contour[c].num_vertices * sizeof(sg_vertex),
          "vertex creation", sg_vertex);
...
void sg_free_tristrip(sg_tristrip *t)
{
  int s;
...
void sg_polygon_to_tristrip(sg_polygon *s, sg_tristrip *t)
{
  sg_polygon c;
  ...
  c.hole= NULL;
  c.contour= NULL;
  sg_tristrip_clip(SG_DIFF, s, &c, t);
}


void sg_tristrip_clip(sg_op op, sg_polygon *subj, sg_polygon *clip,
                       sg_tristrip *result)
{
  sb_tree       *sbtree= NULL;
  it_node       *it= NULL, *intersect;
  ...
  int            vclass, bl, br, tl, tr;
  long double        *sbt= NULL, xb, px, nx, yb, yt, dy, ix, iy;
  ...
  {
    MALLOC(result->strip, result->num_strips * sizeof(sg_vertex_list),
           "tristrip list creation", sg_vertex_list);
    ...
    for (tn= tlist; tn; tn= tnn)
    {
      ...
      if (tn->active > 2)
      {
        ...
        MALLOC(result->strip[s].vertex, tn->active * sizeof(sg_vertex),
               "tristrip creation", sg_vertex);
        v= 0;
        if (INVERT_TRISTRIPS)
        {
          lt= tn->v[RIGHT];
