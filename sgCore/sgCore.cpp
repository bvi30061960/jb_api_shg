...
char           global_temparary_directory[MAXPATH];   // for ANDROID
...
void      stmbx()
{
    #ifdef  TRIAL_VERSION
    {
        ...
        MessageBox(NULL, "sgCore Trial Version", "Message",MB_OK|MB_ICONERROR) ;
#endif
        
#if (SG_CURRENT_PLATFORM==SG_PLATFORM_MAC)
        const char* message = "sgCore Trial Version";
        const char* header = "TRIAL";
...
bool  sgInitKernel(const char* temp_directory)
{
    if (temp_directory)
    {
        strcpy(global_temparary_directory, temp_directory);
    }
    else
    {
      #if (SG_CURRENT_PLATFORM==SG_PLATFORM_WINDOWS)
		GetTempPathA(MAX_PATH, global_temparary_directory );
      #else
        strcpy(global_temparary_directory, "/usr/temp");
      #endif
    }
...
void*    sgPrivateAccess(int Ftype, void* fVoid, void* sVoid)
{
	...
	case 1985: // void  AllocMemoryForBRepPiecesInBRep
		{
			...
			memset(brep->m_pieces,0,number_of_pieces*sizeof(sgCBRepPiece*));
	...
	case  2019:
	{
		global_loadedFileHeader.major_ver = 2;
	}
	break;
	case  2020:
	{
